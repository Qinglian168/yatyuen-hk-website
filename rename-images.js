...
// Get all generated images sorted by timestamp (which is in the filename)
const imgDir = './assets/products';
const files = fs.readdirSync(imgDir).filter(f => f.endsWith('.png')).sort();

// Product ID order (matching products.js data)
const productOrder = [
...
if (files.length !== productOrder.length) {
  console.error('Mismatch! Files:', files.length, 'Products:', productOrder.length);
  process.exit(1);
}

// Rename files
for (let i = 0; i < files.length; i++) {
  const oldPath = path.join(imgDir, files[i]);
  const newPath = path.join(imgDir, productOrder[i] + '.png');
  fs.renameSync(oldPath, newPath);
  console.log(`${files[i]} -> ${productOrder[i]}.png`);
}

console.log('Done! All files renamed.');
